...
export const TechStack = ({stack}: { stack?: string[] }) => {
  ...
  return (
    <div className="">
      <h3 className="text-lg font-mono text-muted-foreground border-b p-4 ">
        TECH STACK
      </h3>
      <div className="flex flex-col w-full">
        {stack.map((techName) => {
          const techDetails = TECH_STACK_DATA[techName] || {
            ...UNKNOWN_TECH,
            name: techName,
          };
          return (
            <TechStackItem key={techDetails.name} techDetails={techDetails}/>
          );
        })}
      </div>
    </div>
  ...
  return (
    <div className="border-b flex flex-row">
      <div className="w-14 flex items-center justify-center py-3">
        <CustomSvg
          src={techDetails.logoWhite}
          className={cn(
            "w-7 h-7 text-black dark:text-white fill-current",
            techDetails.className,
          )}
          style={""}
        />
      </div>
